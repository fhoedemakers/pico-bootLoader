...
# hwconfig and drop the resulting UF2s into uf2/<hwconfig>/<prog_name>.uf2.
#
# Mirrors the "Clone and build the emulators (bootloader branch)" step of
# .github/workflows/BuildAndRelease.yml, but interactive and scoped to a single
# hwconfig + a user-chosen branch for emulator repos and pico_shared.
set -euo pipefail
...
[ -f "$LOADER_DIR/uf2/emulators.txt" ] || die "uf2/emulators.txt not found (run from pico_emuLoader root)"
...
echo
read -r -p "Pick a HW_CONFIG number (e.g. ${HWCONFIGS[0]}, ${HWCONFIGS[2]}, ${HWCONFIGS[5]}): " hw_sel
HWCONFIG=""
for hw in "${HWCONFIGS[@]}"; do
    if [ "$hw_sel" = "$hw" ]; then
        HWCONFIG="$hw"
...
[ -n "$HWCONFIG" ] || die "invalid HW_CONFIG: '$hw_sel' (must be one of: ${HWCONFIGS[*]})"
info "Selected HW_CONFIG=$HWCONFIG (${HW_DESC[$HWCONFIG]})"

# Check PICO_PIO_USB_PATH if the chosen config needs it.
for c in "${PIOUSB_CONFIGS[@]}"; do
    if [ "$c" = "$HWCONFIG" ]; then
        [ -n "${PICO_PIO_USB_PATH:-}" ] || die "HW_CONFIG=$HWCONFIG requires PIO USB; set PICO_PIO_USB_PATH"
        [ -r "${PICO_PIO_USB_PATH}/src/pio_usb.h" ] || die "PICO_PIO_USB_PATH ($PICO_PIO_USB_PATH) does not look like a Pico-PIO-USB checkout"
        break
    fi
done
...
build_one_emulator() {
    ...
    hr
    echo " $prog  ($repo @ $EMU_BRANCH, pico_shared @ $SHARED_BRANCH)"
    echo " clone -> $dest"
    ...
    info "[$prog] cloning ${repo}@${EMU_BRANCH}"
    if ! git clone --branch "$EMU_BRANCH" --no-recurse-submodules --depth 1 \
            "https://github.com/${GITHUB_OWNER}/${repo}.git" "$dest"; then
        elapsed=$(( SECONDS - t0 ))
        echo "SKIP:no '$EMU_BRANCH' branch in $repo|$elapsed" > "$status_file"
        return 0
    fi
...
# Track outcomes for live progress reporting.
